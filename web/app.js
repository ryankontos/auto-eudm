...
function pcToolkitMappings() {
  return Array.isArray(state.preferences?.pc_toolkit_model_mappings)
    ? state.preferences.pc_toolkit_model_mappings
    : [];
}
...
function pcToolkitSuggestedStatus(request, model) {
  ...
  if (!mapping) return "";
  return request.kind === "user" || request.group === "Deployments"
    ? mapping.user_status || ""
    : mapping.location_status || "";
}
...
function queueSearchText(request) {
  ...
  return [
    ...
    request?.pc_toolkit?.serial?.primary?.model,
    request?.pc_toolkit?.serial?.primary?.status,
    request?.pc_toolkit?.serial?.primary?.assigned_user?.login,
    request?.request_id,
    ...userInfo,
    ...location,
...
async function validateBulkSerials({ force = false, requests = null, render = true, serialsByRequest = null } = {}) {
  ...
  bulkRequests.forEach((request) => {
    ...
    updateBulkValidationSummary(request);
    if (request === selectedRequest()) refreshBulkValidationButton(request);
    void enrichBulkRequest(request, { render });
  });
  if (render) renderAll();
  else {
...
async function loadSerialSuggestions(request, query, { requireSelection = true } = {}) {
  ...
  try {
    ...
    if (exact) void enrichRequest(request);
    refreshSelectedValidation();
    updateLookupControlStates(request);
    renderQueue();
  } catch (error) {
...
async function validateUserAfterPause(request, returning = false) {
  ...
  try {
    ...
    if (autoSelectedResult) void enrichRequest(request);
    refreshSelectedValidation();
    updateLookupControlStates(request);
    renderQueue();
  } catch (error) {
...
function updateNoteInlineMarkup(value) {
  return escapeHtml(value)
    .replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>")
    .replace(/`([^`]+)`/g, "<code>$1</code>");
}

function updateNoteMarkup(note) {
  const lines = String(note?.markdown || "").trim().split(/\r?\n/);
  let title = "";
  const blocks = [];
  let paragraph = [];
  let bullets = [];
  const flushParagraph = () => {
    ...
    blocks.push(`<p>${paragraph.map(updateNoteInlineMarkup).join(" ")}</p>`);
    paragraph = [];
  };
  const flushBullets = () => {
    if (!bullets.length) return;
    blocks.push(`<ul>${bullets.map((item) => `<li>${updateNoteInlineMarkup(item)}</li>`).join("")}</ul>`);
    bullets = [];
  };
  ...
  flushBullets();
  if (!title) {
    title = String(note?.file || "Update").replace(/\.md$/i, "").replace(/[-_]+/g, " ");
  }
  return `<article class="update-note"><h3>${escapeHtml(title)}</h3>${blocks.join("")}</article>`;
}

function renderUpdateReview(status = state.serviceStatus) {
  const notes = Array.isArray(status?.update_notes) ? status.update_notes : [];
  const branch = String(status?.branch || "stable");
  const channel = status?.update_channel === "development" ? "Development builds" : "Stable releases";
  const behindCount = Number(status?.behind_count || 0);
  $("#updateNotesSummary").textContent = behindCount > 0
    ? `${behindCount} commit${behindCount === 1 ? "" : "s"} available on ${channel} (${branch}).`
    : `Switch to ${channel} (${branch}).`;
  $("#updateNotesContent").innerHTML = notes.length
    ? notes.map(updateNoteMarkup).join("")
    : '<p class="update-notes-empty">No update notes are available for this change.</p>';
  ...
  const constraintElement = $("#updateNotesConstraint");
  constraintElement.textContent = constraint;
  constraintElement.hidden = !constraint;
  const updateButton = $("#confirmUpdateButton");
  updateButton.disabled = !status?.update_available
    || Boolean(status?.updating || (status?.checking && status?.manual_check))
    || activeSubmissions > 0
    || status?.working_tree_clean === false;
  updateButton.setAttribute("aria-busy", String(Boolean(status?.updating)));
  updateButton.querySelector("span").textContent = status?.updating ? "Updating…" : "Update now";
}

function openUpdateReview() {
  if (!state.serviceStatus?.update_available) return;
  renderUpdateReview();
  if (!$("#updateNotesDialog").open) $("#updateNotesDialog").showModal();
}

function renderServiceStatus(status) {
  if (!status || typeof status !== "object") return;
  state.serviceStatus = status;
  const channel = status.update_channel || state.preferences?.update_channel || "stable";
  const branch = String(status.branch || (channel === "development" ? "main" : "stable"));
  const changed = Boolean(status.update_available);
  const manualCheck = Boolean(status.checking && status.manual_check);
  const busy = Boolean(manualCheck || status.updating);
  const activeSubmissions = Number(status.active_submissions || 0);
  const updateButton = $("#updateAvailableButton");
  if (updateButton) {
    updateButton.hidden = !changed;
    updateButton.disabled = Boolean(status.updating);
    updateButton.title = "Review update notes";
  }
  ...
  if (checkButton) {
    checkButton.disabled = status.updating || manualCheck;
    checkButton.textContent = manualCheck ? "Checking…" : "Check";
  }
  const headerCheckButton = $("#checkUpdatesHeaderButton");
  if (headerCheckButton) {
    headerCheckButton.disabled = Boolean(status.updating);
    headerCheckButton.removeAttribute("aria-busy");
    headerCheckButton.title = status.update_error || "Check for updates";
  }
  ...
  if (applyButton) {
    ...
    applyButton.querySelector("span").textContent = status.updating ? "Updating…" : "Review update";
  ...
  const channelInput = $("#updateChannelInput");
  if (channelInput && !channelInput.matches(":focus")) {
    channelInput.value = state.preferences?.update_channel || status.update_channel || "stable";
  }
  renderUpdateReview(status);
...
function pcToolkitMappingRow(mapping = {}) {
  const userOptions = [{ value: "", label: "No user suggestion" }, ...(state.config?.user_statuses || [])];
  const locationOptions = [{ value: "", label: "No location suggestion" }, ...(state.config?.location_statuses || [])];
  const options = (items, selected) => items.map((item) => `<option value="${escapeHtml(item.value)}" ${item.value === selected ? "selected" : ""}>${escapeHtml(item.label)}</option>`).join("");
  return `<div class="pc-toolkit-mapping-row">
    <label>Model<input data-pc-toolkit-model type="text" value="${escapeHtml(mapping.model || "")}" placeholder="MacBook Pro (14-inch, 2023)"></label>
    <label>User deployment<select data-pc-toolkit-user-status>${options(userOptions, mapping.user_status || "")}</select></label>
    <label>Location deployment<select data-pc-toolkit-location-status>${options(locationOptions, mapping.location_status || "")}</select></label>
    <button class="icon-button" data-pc-toolkit-remove-mapping type="button" aria-label="Remove model mapping"><i data-lucide="x"></i></button>
  </div>`;
}

function renderPcToolkitMappings(extra = null) {
  const wrapper = $("#pcToolkitMappings");
  if (!wrapper) return;
  const mappings = pcToolkitMappings().map((mapping) => ({ ...mapping }));
  (state.pcToolkitStatus?.models || []).forEach((model) => {
    if (model && !mappings.some((mapping) => pcToolkitKey(mapping.model) === pcToolkitKey(model))) {
      mappings.push({ model, user_status: "", location_status: "" });
    }
  });
  if (extra?.model && !mappings.some((mapping) => pcToolkitKey(mapping.model) === pcToolkitKey(extra.model))) mappings.push(extra);
  wrapper.innerHTML = mappings.length
    ? mappings.map(pcToolkitMappingRow).join("")
    : '<div class="pc-toolkit-mapping-empty">No model suggestions saved yet.</div>';
  wrapper.querySelectorAll("[data-pc-toolkit-remove-mapping]").forEach((button) => button.addEventListener("click", () => {
    button.closest(".pc-toolkit-mapping-row")?.remove();
    if (!wrapper.querySelector(".pc-toolkit-mapping-row")) wrapper.innerHTML = '<div class="pc-toolkit-mapping-empty">No model suggestions saved yet.</div>';
  }));
  refreshIcons(wrapper);
}

function readPcToolkitMappings() {
  return $$("#pcToolkitMappings .pc-toolkit-mapping-row").map((row) => ({
    model: row.querySelector("[data-pc-toolkit-model]").value.trim().replace(/\s+/g, " "),
    user_status: row.querySelector("[data-pc-toolkit-user-status]").value,
    location_status: row.querySelector("[data-pc-toolkit-location-status]").value,
  })).filter((mapping) => mapping.model || mapping.user_status || mapping.location_status);
}

function renderPcToolkitStatus() {
  const status = state.pcToolkitStatus || {};
  const title = $("#pcToolkitStatusTitle");
  const detail = $("#pcToolkitStatusDetail");
  ...
  title.textContent = connected ? "PC Toolkit ready" : status.state === "connecting" ? "Connecting…" : "Not connected";
...
function pcToolkitAssignedLabel(device) {
  const person = device?.assigned_user;
  if (!person) return "";
  return person.name && person.login && pcToolkitKey(person.name) !== pcToolkitKey(person.login)
    ? `${person.name} (${person.login})`
    : person.name || person.login || "";
}

async function pcToolkitEnrichQueries(queries, { fresh = false, onBatch = null } = {}) {
...
function pcToolkitFactsMarkup(result, { compact = false } = {}) {
...
function renderRequestPcToolkitDetails(request) {
  const wrapper = $("#pcToolkitRequestDetails");
  if (!wrapper) return;
  if (request?.kind === "bulk_location") {
    const results = Object.values(request.pc_toolkit?.bulk || {}).filter((result) => result?.primary);
  ...
  wrapper.querySelector("[data-pc-toolkit-use-status]")?.addEventListener("click", (event) => {
    recordAppEdit();
    request.status = event.currentTarget.dataset.pcToolkitUseStatus;
    renderAll();
  });
  wrapper.querySelector("[data-pc-toolkit-map-model]")?.addEventListener("click", (event) => {
    openSettings({ tab: "pc-toolkit", model: event.currentTarget.dataset.pcToolkitMapModel });
  });
}

async function enrichBulkRequest(request, { render = true } = {}) {
  if (!state.preferences.pc_toolkit_enabled || request?.kind !== "bulk_location" || !request.serials?.length) return;
  const epoch = Number(request.pc_toolkit_epoch || 0) + 1;
  request.pc_toolkit_epoch = epoch;
  try {
    const payload = await pcToolkitEnrichQueries(request.serials);
    if (request.pc_toolkit_epoch !== epoch) return;
    request.pc_toolkit = { bulk: payload.results || {}, enriched_at: new Date().toISOString() };
  } catch (_) {
  } finally {
    if (render && selectedRequest() === request) renderRequestPcToolkitDetails(request);
    if (render) renderQueue();
  }
}

async function enrichRequest(request, { render = true, includeUser = true } = {}) {
  if (!state.preferences.pc_toolkit_enabled || !request) return;
  const serial = String(request.serials?.[0] || "").trim();
  const username = String(request.user || request.returning_user || "").trim();
  const queries = [serial, ...(includeUser && username ? [username] : [])].filter((value) => value.length >= 2);
  ...
  if (render && selectedRequest() === request) renderRequestPcToolkitDetails(request);
  try {
    const payload = await pcToolkitEnrichQueries(queries);
    if (request.pc_toolkit_epoch !== epoch) return;
    const results = payload.results || {};
    request.pc_toolkit = {
    ...
    // Enrichment is optional and must never turn into a validation failure.
  } finally {
    if (request.pc_toolkit_epoch === epoch) request.pc_toolkit_loading = false;
...
function pcToolkitConflictFor(request) {
  const result = request?.pc_toolkit?.serial;
  const device = result?.primary;
...
function openSettings({ tab = "", model = "" } = {}) {
  ...
  void refreshPcToolkitStatus().then(() => renderPcToolkitMappings(model ? { model, user_status: "", location_status: "" } : null));
}
...
function resetImportDialog(mode = state.importMode || "deploy") {
  ...
  $("#importPreviewSearchInput").value = "";
...
function returnedSerialEditorMarkup(payload) {
  ...
  const enteredLabel = summary.entered.length
    ? "Ready to match entered serials"
    : "Paste one or more serials to match";
  const unmatchedLabel = summary.unmatched.length
    ? " · Some serials have no match yet"
    : "";
  return `<div class="alm-returned-serials import-preview-returned-serials">
    <div class="alm-returned-serials-heading"><div><strong>Returned serials on hand</strong><small>Match a returned laptop to a user’s currently deployed PC Toolkit device.</small></div></div>
...
function almDeploymentDateLabel(value) {
...
function missingUsernameWarningId(warning) {
  if (!warning.id) {
    warning.id = `missing-user-${warning.date || "date"}-${warning.row_number || "row"}-${pcToolkitKey(warning.serial)}`;
  }
  return warning.id;
}

function missingUsernameDeploymentRequest(warning, username) {
  return {
    id: uid(),
    kind: "user",
    serials: [String(warning.serial || "").trim()],
    status: warning.status_preselected ? String(warning.status || "").trim() : "",
    user: username,
    user_info: null,
    ...
    returning_user_validation: "empty",
    group: "Deployments",
    source: `${state.workbook?.filename || "ALM Workbook"} · ${$("#sheetInput")?.value || "Workbook"}`,
    device_allocation: warning.device_allocation || "",
    new_asset_status: warning.new_asset_status || "",
    first_name: warning.first_name || "",
    last_name: warning.last_name || "",
    has_returned_device_serial: warning.has_returned_device_serial === true,
    has_pending_return_serial: warning.has_pending_return_serial === true,
    new_joiner: warning.new_joiner === true,
    deployment_date: warning.date || "",
    included: true,
    import_validation: "pending",
    import_error: "",
    import_failed_fields: [],
    ...
    cached_serial_verification: false,
    cached_user_verification: false,
    manual_username_source_id: missingUsernameWarningId(warning),
  };
}

function missingUsernameEditorMarkup(warnings) {
  if (!warnings.length) return "";
  return `<div class="import-missing-users" role="status">
    <div class="import-missing-users-heading"><div><strong>Username needed</strong><small>Add the receiving user to keep these deployment serials in the review.</small></div><span>${warnings.length}</span></div>
    <div class="import-missing-users-list">${warnings.map((warning) => {
      const id = missingUsernameWarningId(warning);
      const dateLabel = almDeploymentDateLabel(warning.date);
      const details = [warning.device_allocation, dateLabel].filter(Boolean);
      return `<div class="import-missing-user-row" data-import-missing-user-row="${escapeHtml(id)}">
        <div class="import-missing-user-device"><strong>${escapeHtml(warning.serial || "No serial")}</strong>${details.length ? `<small>${escapeHtml(details.join(" · "))}</small>` : ""}</div>
        <div class="import-missing-user-form"><input type="text" data-import-missing-user="${escapeHtml(id)}" value="${escapeHtml(warning.username_input || "")}" placeholder="Enter username" aria-label="Username for ${escapeHtml(warning.serial || "deployment")}" autocomplete="off"><button class="button secondary compact" type="button" data-import-missing-user-add="${escapeHtml(id)}">Add user</button></div>
        ${warning.error ? `<small class="import-missing-user-error">${escapeHtml(warning.error)}</small>` : ""}
      </div>`;
    }).join("")}</div>
  </div>`;
}
...
function manualReturnEditorMarkup(source, payload) {
  ...
  const candidateMarkup = !added && visibleDevices.length
    ? `<div class="pc-toolkit-return-candidates"><div class="pc-toolkit-return-heading"><span>Devices currently deployed to this user</span></div>${visibleDevices.map(({ device, pending }) => {
        const model = String(device.model || device.name || "").trim();
        return `<div class="pc-toolkit-return-candidate"><div class="pc-toolkit-return-device"><strong>${escapeHtml(device.serial)}</strong>${model ? `<span>${escapeHtml(model)}</span>` : ""}<small>Deployed</small></div>${pending ? `<span class="pc-toolkit-return-current-state">Already ${escapeHtml(device.status || "Pending Return")}</span>` : `<div class="pc-toolkit-return-actions"><button class="button secondary compact" type="button" data-pc-toolkit-return-candidate="${escapeHtml(source.id)}" data-candidate-serial="${escapeHtml(device.serial)}" data-candidate-model="${escapeHtml(device.model || "")}" data-candidate-type="returned_devices">Returned</button><button class="button secondary compact" type="button" data-pc-toolkit-return-candidate="${escapeHtml(source.id)}" data-candidate-serial="${escapeHtml(device.serial)}" data-candidate-model="${escapeHtml(device.model || "")}" data-candidate-type="pending_returns">Pending return</button></div>`}</div>`;
      }).join("")}</div>`
...
function importReviewSnapshot(payload = state.importPreview) {
  ...
  return {
    requests: (payload.requests || []).map((request) => ({
      ...
      // complete object for undo/redo rather than only editable fields.
      request: request.manual_return_source_id || request.manual_username_source_id
        ? cloneImportHistoryValue(request)
        : null,
...
function restoreImportReviewSnapshot(snapshot) {
  ...
  (snapshot.requests || []).forEach((item) => {
    ...
    const isManualReturn = Boolean(
      request.manual_return_source_id
      || item.request?.manual_return_source_id
      || request.manual_username_source_id
      || item.request?.manual_username_source_id,
    );
...
async function rememberPcToolkitMapping(request) {
  const model = pcToolkitModelFor(request);
  if (!model || !request.status) return;
  const mappings = pcToolkitMappings().map((mapping) => ({ ...mapping }));
  let mapping = mappings.find((item) => pcToolkitKey(item.model) === pcToolkitKey(model));
  if (!mapping) {
    mapping = { model, user_status: "", location_status: "" };
    mappings.push(mapping);
  }
  if (request.kind === "user" || request.group === "Deployments") mapping.user_status = request.status;
  else mapping.location_status = request.status;
  try {
    state.preferences = await api("/api/preferences", {
      method: "POST",
      body: JSON.stringify({ pc_toolkit_model_mappings: mappings }),
    });
    request.pc_toolkit_suggested_status = request.status;
    renderImportPreview();
    toast(`Saved the ${model} suggestion.`, "success");
  } catch (error) { toast(error.message, "error"); }
}
...
function importReviewSearchQuery() {
  return String($("#importPreviewSearchInput")?.value || "").trim().toLocaleLowerCase();
}

function importReviewMatches(request, query = importReviewSearchQuery()) {
  if (!query) return true;
  const toolkitDevices = [
    request?.pc_toolkit?.serial?.primary,
    request?.pc_toolkit?.primary,
    ...(request?.pc_toolkit?.user?.devices || []),
  ].filter(Boolean);
  const values = [
    ...(request.serials || []),
    request.serial,
    ...
    ...toolkitDevices.flatMap((device) => [
      ...
      device.model,
      device.status,
      device.assigned_user?.login,
      device.assigned_user?.name,
    ]),
  ];
...
function renderBacklogPreview(payload) {
  ...
  const emptyMessage = query
    ? "No review rows match your search."
    : Number(counts.already_deployed || 0) > 0
...
function renderImportPreview() {
  ...
  $("#importPreviewList").innerHTML = manualReturnSection + (manualReturnEntries.length ? "" : returnedSerialsMarkup) + groups.map((group) => {
    ...
    const missingUsernameWarnings = group.key === "Deployments"
      ? (Array.isArray(payload.warnings?.missing_username_deployments) ? payload.warnings.missing_username_deployments : [])
        .filter((warning) => !warning.added_request_id
          || !payload.requests.some((request) => request.id === warning.added_request_id))
        .filter((warning) => importReviewMatches({
          ...warning,
          serials: [warning.serial],
        }, query))
      : [];
  ...
  const addMissingUsername = (warningId) => {
    const warnings = Array.isArray(payload.warnings?.missing_username_deployments)
      ? payload.warnings.missing_username_deployments
      : [];
    const warning = warnings.find((item) => missingUsernameWarningId(item) === warningId);
    if (!warning) return;
    const input = $("#importPreviewList").querySelector(`[data-import-missing-user="${CSS.escape(warningId)}"]`);
    const username = String(input?.value || warning.username_input || "").trim();
    warning.username_input = username;
    if (username.length < 2) {
      warning.error = "Enter the receiving user’s username.";
      renderImportPreview();
      return;
    }
    if (payload.requests.some((request) => (request.serials || []).some((serial) => pcToolkitKey(serial) === pcToolkitKey(warning.serial)))) {
      warning.error = "That serial is already included in this review.";
      renderImportPreview();
      return;
    }
    recordImportEdit();
    const request = missingUsernameDeploymentRequest(warning, username);
    payload.requests.push(request);
    warning.added_request_id = request.id;
    warning.error = "";
    renderImportPreview();
    updateImportPrepareButton(payload);
    saveCurrentImportDraft();
  ...
  elements.historyList.innerHTML = matchingRuns.map((run) => {
    ...
    const entries = (run.entries || []).map((entry) => {
      ...
      const serialMarkup = serials.length
        ...
        : "No serial";
      const pcModel = pcToolkitModelFor(entry);
...
function bindEvents() {
  ...
  $("#addPcToolkitMappingButton").addEventListener("click", () => {
    ...
    $("#pcToolkitMappings").insertAdjacentHTML("beforeend", pcToolkitMappingRow());
    const row = $("#pcToolkitMappings .pc-toolkit-mapping-row:last-child");
    row.querySelector("[data-pc-toolkit-remove-mapping]").addEventListener("click", () => row.remove());
    refreshIcons(row);
  ...
  $("#clearPcToolkitCacheButton").addEventListener("click", async () => {
    try {
      ...
      toast("PC Toolkit cache cleared.", "success");
      await refreshPcToolkitStatus();
    } catch (error) { toast(error.message, "error"); }
  });
  $("#clearPcToolkitModelsButton").addEventListener("click", async () => {
    try {
      state.preferences.pc_toolkit_model_mappings = [];
      await api("/api/preferences", { method: "POST", body: JSON.stringify({ pc_toolkit_model_mappings: [] }) });
      const response = await api("/api/pc-toolkit/models", { method: "DELETE" });
      state.pcToolkitStatus = response.status;
      renderPcToolkitMappings();
      renderPcToolkitStatus();
      toast("PC Toolkit model list cleared.", "success");
    } catch (error) { toast(error.message, "error"); }
  });
  ...
  $("#saveSettingsButton").addEventListener("click", async () => {
    ...
    const preferences = {
      ...
      update_channel: $("#updateChannelInput").value || state.preferences.update_channel || "stable",
  ...
  $("#importPreviewSearchInput").addEventListener("input", () => {
    if (state.importPreview) renderImportPreview();
  });
