...
- Helix's `sessionstatus` response is not proof of an authenticated API session. Connection setup must first create the DWP web-client session through `/dwp/restapi/users/sessions`, then verify both the EUDM service catalogue and carts API, and finally verify the transferred API client before reporting success. Helix API requests use the site-root origin/referrer and the authenticating Chrome user agent.
