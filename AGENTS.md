...
- PC Toolkit device lookups default to a real Chrome page using the same dedicated profile as sign-in. Puppeteer is an optional real-Chrome transport for machines where it is more reliable; Direct API remains an explicit fallback in Settings. Portal role/heartbeat authentication does not use a fabricated serial probe; the first real lookup validates the device API. Helix always authenticates first and explicitly pauses PC Toolkit before reauthentication so the optional service never holds the shared profile while Helix needs it. Browser-backed PC Toolkit bulk lookups are serialized, retry transient failures, refresh an expired bearer token in-page, and may fall back to cached enrichment. The Puppeteer transport uses the tracked `puppeteer-core` dependency and `src/auto_eudm/pc_toolkit_puppeteer.cjs` JSON-lines bridge.
