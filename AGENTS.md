...
- PC Toolkit device lookups default to a real Chrome page using the same dedicated profile as sign-in; Direct API remains an explicit fallback in Settings. Browser requests retain the production fetch shape captured from its portal, including the bearer token returned by the portal session heartbeat and the elevated-role header. Portal role discovery alone is not a successful connection: verify the device API after browser authentication before reporting PC Toolkit as ready. Helix and PC Toolkit share a process-local profile lock and clean temporary blank tabs so simultaneous startup does not repeatedly open unusable windows.
