...
- PC Toolkit device lookups default to a real Chrome page using the same dedicated profile as sign-in. Puppeteer is an optional real-Chrome transport for machines where it is more reliable; Direct API remains an explicit fallback in Settings. Portal role/heartbeat authentication does not use a fabricated serial probe; the first real lookup validates the device API. Helix and PC Toolkit share a process-local profile lock and reuse the startup tab so authentication does not create a trail of unusable `about:blank` windows. The Puppeteer transport uses the tracked `puppeteer-core` dependency and `src/auto_eudm/pc_toolkit_puppeteer.cjs` JSON-lines bridge.
